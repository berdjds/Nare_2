    ...
    const fetchTours = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Filter only active daily tours
          const dailyTours = data.filter((tour: Tour) => 
            tour.category === 'daily' && tour.isActive !== false
          );
          setTours(dailyTours);
        }
      } catch (error) {
    ...
    <>
      ...
      <div className="container mx-auto px-4 py-16">
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-lg text-gray-600">No daily tours available at the moment.</p>
          </div>
        ) : (
